			...
			for sub_words in thesaurus[target_w]:
				try:
					...
					try:
						...
						continue
				score = 0.0
				for ind in range(target_pos - 5, target_pos + 5):
					vec_t_c = []
					...
					if len(vec_t_c) > 1:
						...
						# print len(vector_of_the_target_word),len(vec_t_c)
						# print "单词分数:",score
						score += cosine_similarity(vector_of_the_sub_word, vec_t_c)
					# print vector_of_the_sub_word,vec_t_c
		...
		for content_sens in jsonSentence:
			...
			for sc in scores:
				if scores[sc]>best_score:
					...
					best_score = scores[sc]
			print "目标ID：",target_id,"\n","最佳词：",best_word
			filename = modeltype+csType+'_output1111111'
			outing = open(filename,'a')
			best_word.pop()
			best_word.pop()
			outputthings = str(target_w)+" "+str(target_id)+' :: '+str(best_word)
				...
				for ind in range(target_pos - 5, target_pos + 5):
					...
					if ind == target_pos or ind < 0:
						...
						try:
							...
							# vec_t_c = [i/prodt for i in prob_l]
						except:
							continue
					...
					if len(vec_t_c) > 1:
						...
						# print vector_of_the_sub_word,vec_t_c
				if score==0:
					print "等于零的词和分：",sub_words,score
				scores[sub_words] = score
			# print 'hahaha'
			best_word = ''
			best_score = 0.0
			print target_w,scores
			for sc in scores:
				if scores[sc]>best_score:
					best_word = sc
					best_score = scores[sc]
			print 'thebest',best_word
			filename = 'aaaaa'+csType+'_output'
			outing = open(filename,'a')
			outputthings = str(target_w)+" "+str(target_id)+' :: '+str(best_word)
			print outputthings
			outing.write(outputthings)
...
if __name__ == "__main__":
	...
	if part == "d":
		...
		# calculate_prob_list_of_tc('home.n','house.n',ldaModel,vectors)
	...
	if part == "e":
		...
		best_substitute(jsonSentence=sens, frequencyVectors=vectors, thesaurus=thesaurus, word2id=word2id, model=ldaModel,
						csType='lda')
	...
	# 	Word2Vec = word2vec("bnc.vert", 0.05, 0.001, 9)
	# if part == 'fuck':
	# 	ldaM = gensim.models.ldamodel.LdaModel.load('lda.model')
